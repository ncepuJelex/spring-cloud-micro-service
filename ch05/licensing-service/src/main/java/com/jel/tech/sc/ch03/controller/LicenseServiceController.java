...
public class LicenseServiceController {
     ...
     * @param orgId
     * @param licenceId
     * @param clientType    Discovery/Rest/Feign
     * @return
     */
    @GetMapping("/{licenceId}/{clientType}")
    public License getLicensesWithClient(@PathVariable("orgId") String orgId,
                                         @PathVariable("licenceId") String licenceId,
                                         @PathVariable("clientType") String clientType) {

        return licenseService.getLicense(orgId, licenceId, clientType);
    }

    @RequestMapping(value="/",method = RequestMethod.GET)
    public List<License> getLicenses(@PathVariable("orgId") String orgId) {

        System.out.println("mock log getLicenses controller method comes in~" +  UserContextHolder.getContext().getCorrelationId());
